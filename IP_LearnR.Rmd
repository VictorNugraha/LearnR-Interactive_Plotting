...
```{r abcdefghijk, exercise = TRUE, exercise.eval = TRUE}
# Cara dplyr
...
```{r parsons2, echo=FALSE}
question_parsons(
  initial = c(
    "ggplot(data  = ..., aes(x = ..., y = ..., text = glue(...)))",
    "labs(...)",
  ...
  fail_if(
    function(x){"ggplot(data  = ..., aes(x = ..., y = ..., text = glue(...)))" %in% x},
    message = "You should not include ggplot(data  = ..., aes(x = ..., y = ..., text = glue(...))) in your answer"
  ),
...
Setelah berhasil mengurutkan dengan benar setiap fungsi yang akan digunakan dari tahapan aggregasi data sampai visualisasi, mari kita coba isi setiap parameter yang dibutuhkan untuk menghasilkan visualisasi yang sama seperti diatas.

```{r abcdefghij, exercise = TRUE, exercise.eval = TRUE}
...
question_parsons(
  ...
  fail_if(
    ...
    message = "You should not include geom_boxplot(fill = ...) in your answer"
  ),
...
```{r abcdefghi, exercise = TRUE, exercise.eval = TRUE}
